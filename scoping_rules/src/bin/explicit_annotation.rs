...
fn main() {
    ...
    failed_borrow();
    // `failed_borrow` contains no references to force `'a` to be longer than lifetime of the
    // function, but `'a` is longer. Because the lifetime is never constrained, it defaults to `'static`.
}
